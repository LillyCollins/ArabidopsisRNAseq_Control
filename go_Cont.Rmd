...
gene2GO <- go %>%
  ...
  mclapply(function(x){   # x: each one of the 26481 elements in the list
    unique(x$GO_terms)    
  }, 
...
```{r modify go results for different input goRes lists}
goRes_filtered <- names(DEGs) %>%  
  lapply(function(x){     # x is each one of the 6 MutantVsWt comparisons
    goRes[[x]] %>% 
      mutate(Group = x) %>% 
      dplyr::select(-notDECount) %>% 
      filter(adjP < 0.05 & shortest_path >= 4) 
      # i modified the codes, filtered within the loop
  }) %>% 
  bind_rows() %>% 
  split(.$Expr) %>%     # split into up and down two sub-lists
  lapply(function(x){
    x %>% 
      group_by(GO_term)
  })
...
dfdown <- 
  goRes_filtered$Down[c(1,2, 6, 7, 13, 14)] %>%  # subset needed data
  # get -log10(p) values, re-assign -LogP to 0 for adjP>= 0.05
  dplyr::mutate("-LogP" = ifelse(adjP < 0.05, -log10(p), 0), 
                .keep = "unused") %>% 
  rstatix::reorder_levels("Group", order = unique(.$Group)) %>% 
...
```{r Save go2GENE for GO enrichment analysis}
